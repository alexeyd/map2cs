...
bool csSaver::SaveSectorLights(iSector *s, iDocumentNode *parent)
{
  ...
  for (int i=0; i<lightList->GetCount(); i++)
  {
    ...
    //Save the light type
    csRef<iDocumentNode> lightTypeNode = CreateNode(lightNode, "type");

    switch(light->GetType())
    {
      case CS_LIGHT_SPOTLIGHT:
        lightTypeNode->CreateNodeBefore(CS_NODE_TEXT)->SetValue("spotlight");
        break;

      case CS_LIGHT_DIRECTIONAL:
        lightTypeNode->CreateNodeBefore(CS_NODE_TEXT)->SetValue("directional");
        break;

      default:
        lightTypeNode->CreateNodeBefore(CS_NODE_TEXT)->SetValue("pointlight");
        break;
    }
    ...
    //Add the transformation tags
    csMatrix3 moveMatrix =
      light->GetMovable()->GetTransform().GetO2T();
    if (!moveMatrix.IsIdentity())
    {
      //Add the move tag
      csRef<iDocumentNode> moveNode = CreateNode(lightNode, "move");
